...
class Policy(nn.Module):
    def __init__(self, obs_space, action_space, base_kwargs=None):
        ...
        img_num_features = obs_space.spaces['img'].shape[0]
        v_num_features = obs_space.spaces['v'].shape[0]
        self.base = CNNBase(img_num_features, v_num_features, **base_kwargs)
    ...
    def forward(self, img, v, rnn_hxs, masks):
        raise NotImplementedError

    def act(self, img, v, rnn_hxs, masks, eps, deterministic=False):
        value, actor_features, rnn_hxs = self.base(img ,v, rnn_hxs, masks)
    ...
    def get_value(self, img, v, rnn_hxs, masks):
        value, _, _ = self.base(img, v, rnn_hxs, masks)
        return value

    def evaluate_actions(self, img, v, rnn_hxs, masks, action):
        value, actor_features, rnn_hxs = self.base(img, v, rnn_hxs, masks)
        dist = self.dist(actor_features)
